...
db=mysql.connector.connect(user='root',password='',port=3306,database='cloudsecurestoragemechanism')
cur=db.cursor()
app=Flask(__name__)
...
@app.route('/myfiles')
def myfiles():
    sql="select id,Filename,Fileencdata from serverone where Receiver='%s'"%(session['receivername'])
    data1=pd.read_sql_query(sql,db)
    db.commit()
    print(data1.id)

    sql = "select id,Filename,Fileencdata from servertwo where Receiver='%s'" % (session['receivername'])
    data2 = pd.read_sql_query(sql, db)
    db.commit()

    sql = "select id,Filename,Fileencdata from serverthree where Receiver='%s'" % (session['receivername'])
    data3 = pd.read_sql_query(sql, db)
    db.commit()
...
def Viewdownload(y=0,z=''):
    ...
    sql = "select id,Filename,Fileencdata from serverone where id='%s' and Filename='%s'" % (y,z)
    cur.execute(sql)
    data1=cur.fetchall()
    db.commit()
    ...
    sql = "select id,Filename,Fileencdata from servertwo where id='%s' and Filename='%s'" % (y,z)
    cur.execute(sql)
    data2 = cur.fetchall()
    db.commit()
    ...
    sql = "select id,Filename,Fileencdata from serverthree where id='%s' and Filename='%s'" % (y,z)
    cur.execute(sql)
    data3 = cur.fetchall()
    db.commit()
    for i in data3:
...
def decryptdata(y=0,z=''):
    if request.method=='POST':
        ...
        sql="select AES_DECRYPT(Fileencdata, 'keys')decrypt from serverone where id='%s' and Filename='%s'"%(y,z)
        cur.execute(sql)
        data1=cur.fetchall()
        db.commit()
        ...
        if data1 ==[]:
            sql = "select AES_DECRYPT(Fileencdata, 'keys')decrypt from servertwo where id='%s' and Filename='%s' "%(y,z)
            cur.execute(sql)
            data2 = cur.fetchall()
            db.commit()
            ...
            if data2 ==[]:
                sql = "select AES_DECRYPT(Fileencdata, 'keys')decrypt from serverthree where id='%s' and Filename='%s'"%(y,z)
                cur.execute(sql)
                data3 = cur.fetchall()
                db.commit()
...
def encryptdata(x=0,y=''):
    ...
    if data1!=[]:
        ...
        name = data1[0][1]
        content = data1[0][2]
        sql="update serverone set Fileencdata=AES_ENCRYPT('%s','keys')"%(content)
        cur.execute(sql)
        db.commit()
        sql = "select * from serverone where id='%s' and Filename='%s'" % (x, y)
        cur.execute(sql)
        ...
        name = data2[0][1]
        content = data2[0][2]
        sql="update servertwo set Fileencdata=AES_ENCRYPT('%s','keys')"%(content)
        cur.execute(sql)
        db.commit()
        sql = "select * from servertwo where id='%s' and Filename='%s'" % (x, y)
        cur.execute(sql)
        ...
        name = data3[0][1]
        content = data3[0][2]
        sql="update serverthree set Fileencdata=AES_ENCRYPT('%s','keys')"%(content)
        cur.execute(sql)
        db.commit()
        sql = "select * from serverthree where id='%s' and Filename='%s'" % (x, y)
        cur.execute(sql)
